...
ALPHA = 0.5;
freq_error = [0 0.02 0.05 0.1];
EbN0dB = 0:10;
BER = zeros(length(EbN0dB),length(freq_error));
...
%==============Filter Design==============%
        ...
        %=========================================%
for freq_error_id = 1:length(freq_error)
    for ebn0db_id = 1:length(EbN0dB)
        snr_max_times = 10^9;
        MAX_ERROR_BITS = 100;
        shift = 8; % default: 8. downsample shift in [0,n-1]
        ...
        for sim_time_cnt = 1:snr_max_times
            ...
            EbN0linear = 10^(EbN0dB(ebn0db_id)/10);
            ...
            k = 1:length(rx_downsampled_data);
            rx_downsampled_data = rx_downsampled_data.*exp(1j*2*pi*freq_error(freq_error_id)*k/number_of_wfrft_carriers);
        ...
        BER(ebn0db_id,freq_error_id) = total_errbit/total_bit;
    end
end
...
hold off
legend( ['\epsilon = 0'],['\epsilon = 0.03'] ,['\epsilon = 0.05'], ['\epsilon = 0.1'] );
FX=xlabel('Eb/N0(dB)');
FY=ylabel('BER');
set(FX,'FontSize',14);
set(FY,'FontSize',14);
